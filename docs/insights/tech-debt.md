...
| 2 | 40 of 44 `# noqa` directives are inert — 39 name rules that `select` never enables (BLE/SLF/ANN/S/PT prefixes) and one is genuinely unused, so the justifications they carry protect nothing; `ruff check --extend-select RUF100` reports all 40 while the committed config reports `All checks passed!` | `marker` | `S` | `pyproject.toml:53`, `src/pneuma/gated.py:186`, `src/pneuma/detect/objective.py:181`, `src/pneuma/team.py:583`, `src/pneuma/casestudy/minelearn.py:872` |
| 3 | `Team` is 876 LOC across 34 methods, mixing prose rendering, run lifecycle, and its own five-method validation suite in one class | `wrong abstraction` | `L` | `src/pneuma/team.py:780`, `src/pneuma/team.py:1122`, `src/pneuma/team.py:1280`, `src/pneuma/team.py:1490-1652` |
...
| 8 | RESOLVED 2026-08-10: the README's component table published stale LOC counts (`team.py` listed at 968 lines against an actual 1655); the README rewrite re-measured every count (`README.md:109-114`) | `dead code adjacent` | `S` | `README.md:109-114`, `src/pneuma/team.py`, `src/pneuma/gated.py`, `src/pneuma/process/agent.py` |
...
- `# noqa: BLE001 — the model can retry or re-scope; see _hiring` — `src/pneuma/team.py:583`
- `# noqa: BLE001 — one dead teammate must not stop the rest` — `src/pneuma/team.py:719`
...
### Two objects that grew past their own docstrings

`Team` (`src/pneuma/team.py:780`) is 876 LOC and 34 methods. It holds prose rendering (`render_brief` at `:1018`, `render_plan` at `:1044`, `render_objections` at `:1084`), run lifecycle (`assemble` at `:1219`, `brief` at `:1240`, `negotiate` at `:1280`, `fork` at `:1458`, `teardown` at `:1469`), and its own five-method validation suite (`:1490-1652`) — three responsibilities that would each be a coherent unit. `TursoMemoryBackend` (`src/pneuma/memory/turso_backend.py:360`) is 860 LOC and 39 methods with 22 SQL statements inline, spanning schema DDL, entry CRUD, embedding, retrieval-quality measurement (`probe_retrieval` at `:718`, `calibrate_ceiling` at `:784`), optimizer callbacks (`_save`/`_recall`/`_consolidate` at `:963-1091`), and LLM tool construction (`tool_provider` at `:1134`). The register ranks these below the suppression smell deliberately: both are internally coherent and well documented, so the consequence of leaving them is slow (every new feature lands in an already-crowded class) rather than sharp. The measurable symptom was that the documentation lost track — an earlier README described `team.py` at 968 lines against an actual 1655, corrected in the 2026-08-10 rewrite (`README.md:114`).

Shows up in:

- `src/pneuma/team.py:780` — `Team`, 876 LOC / 34 methods
- `src/pneuma/team.py:1122` — `execute`, 96 LOC in one method
...
Cost: `L` — splitting either means moving a public surface, and `Team`'s validation methods are coupled to its own fields.
