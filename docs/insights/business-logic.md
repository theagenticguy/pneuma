...
2. **Explicit guard clauses** — 53 `raise ValueError` sites and 135 `raise` statements across `src/`, concentrated in `team.py` (17), `process/ir.py` (17), `memory/turso_backend.py` (12), `casestudy/transcriptlog.py` (12), `gated.py` (11), and `recall.py` (9).
...
| A `Team` subclass must override `members`, `briefing`, `lead_function`, and `oracle`; every omission is named at once | pneuma (kernel) | `src/pneuma/team.py:897`, `1629-1652` | Reject at construction — `RuntimeError` |
| `negotiation_rounds` may not be negative | pneuma (kernel) | `src/pneuma/team.py:1614-1627` | Reject at construction — `RuntimeError`; a negative value would behave as 0 |
| Two cast members may not share a name | pneuma (kernel) | `src/pneuma/team.py:1490-1520` | Reject pre-spawn — `RuntimeError`; briefings are keyed by name and one would overwrite the other |
| Every member's briefing failing is not recoverable; the lead is not spawned | pneuma (kernel) | `src/pneuma/team.py:1522-1559` | Reject — `RuntimeError`; a single failure is rendered as text instead |
| A post-condition's first parameter may not share a name with a lead parameter | pneuma (kernel) | `src/pneuma/team.py:1561-1597` | Reject at wiring — `RuntimeError` |
| A `Member`'s capability must take at least one positional parameter, or a briefing has nowhere to go | pneuma (kernel) | `src/pneuma/team.py:151-170` | Reject — `RuntimeError` |
| A member that already carries a `config_hook` may not be equipped with a second | pneuma (kernel) | `src/pneuma/team.py:195-199` | Reject — `RuntimeError`; the runtime calls exactly one hook per cycle |
| A lead carrying its own `config_hook` may not be combined with a hiring surface | pneuma (kernel) | `src/pneuma/team.py:1428-1435` | Reject — `RuntimeError`, refused loudly rather than resolved by precedence |
| A `DynamicAgent`'s instructions may not be empty | pneuma (kernel) | `src/pneuma/team.py:257-262` | Reject — `ValueError` (caller path) |
| A hired role must exist in the catalog | pneuma (kernel) | `src/pneuma/team.py:495-496` | Return text — `"error: no such role ..."`, so the model retries |
| A hire's name must be unused | pneuma (kernel) | `src/pneuma/team.py:497-498`, `540-541` | Return text |
| A `hire_dynamic` call must supply non-empty instructions | pneuma (kernel) | `src/pneuma/team.py:535-539` | Return text |
| `delegate` and `dismiss` must name a hire that exists | pneuma (kernel) | `src/pneuma/team.py:576-580`, `597-599` | Return text |
| A posted discovery's `kind` must be one of the four `DISCOVERY_KINDS` | pneuma (kernel) | `src/pneuma/team.py:629`, `755-757` | Return text; the model picks a real kind and posts again |
...
| A hire reserves its name and its headcount before it awaits anything, rolling back if the spawn raises | Application code — `commission` runs synchronously up to the first suspension | `src/pneuma/team.py:378-387`, `453-483`, `506-511` |
| A worklog post appends to `entries` before any delivery is awaited, so two concurrent posts cannot drop one | Application code — `Worklog.post` | `src/pneuma/team.py:653-658`, `693-712` |
| One failing worklog channel never stops the rest; the failure is recorded on the entry | Application code — `_deliver` | `src/pneuma/team.py:660-664`, `714-722` |
| A worklog poster is excluded from its own fan-out | Application code — `post` | `src/pneuma/team.py:693-711` |
| `register` replays every prior entry into a newly opened channel, so registration order does not matter | Application code | `src/pneuma/team.py:677-691` |
| The roster and worklog live for exactly one run and are replaced as `type(self.x)()` at the top of `execute`, preserving a subclass's narrowed type | Application code | `src/pneuma/team.py:797-802`, `878-895`, `1155-1156` |
| Every member, every hire, and the lead are retired even when a phase raised — and again on external termination | Application code — `finally` in `execute` plus `teardown` | `src/pneuma/team.py:1189-1199`, `1469-1480` |
| `dismiss` retires before unregistering, so a failed retire leaves the thread reachable to a retry | Application code | `src/pneuma/team.py:601-611` |
| The briefing barrier waits for every member, so a verdict's evidence does not depend on scheduling | Application code — `asyncio.gather(..., return_exceptions=True)` | `src/pneuma/team.py:1245-1256`, `1265-1268` |
| A team with no members returns negotiation-silent rather than recording a vacuous unanimity | Application code | `src/pneuma/team.py:1314-1319` |
| Attaching the oracle *prepends* it to the lead's own post-conditions rather than replacing them | Application code — `_gated_lead` | `src/pneuma/team.py:1402-1427` |
| `Team.fork` is refused outright: a fork would share live members and one mutable roster | Application code — `NotImplementedError` | `src/pneuma/team.py:1458-1467` |
...
| A published `investigation.json` keeps its nine keys in order; a team that never negotiated and never used a worklog drops those keys rather than emitting empty ones | Application code — `model_serializer` plus serialization aliases | `src/pneuma/team.py:340-356`, `src/pneuma/demo/warroom.py:41-69` |
...
| Negotiation unanimity | approvals, objections | `len(approved) == len(objections)`; else `revised` or `cap_reached` | `src/pneuma/team.py:1336-1359` |
| Approval detection | a member's answer text | containment of `APPROVED`, with a `BRIEFING_ERROR` prefix vetoing | `src/pneuma/team.py:1071-1082` |
| Team token and turn rollup | pre-spawn event baseline, subtree usage | input/output tokens and turn count | `src/pneuma/team.py:1154`, `1202` |
...
- **Hiring headcount cap:** a lead may hold at most `max_hires` subagents at once, and the cap is checked before the recruit is built so a rejected hire spawns nothing. `Team` defaults to 3 and the standalone `hiring_tools` seam defaults to 4, so a caller wiring the seam directly gets a looser budget than a `Team` subclass. `src/pneuma/team.py:499-504`, `542-544`, `813`, `366`.
- **Reserve-before-spawn:** the cap and the name are both spent synchronously before any await, because the runtime's tool executor is concurrent and two `hire` calls in one assistant turn would otherwise both pass the check. `src/pneuma/team.py:378-387`, `453-475`.
- **Catalog as the hiring allowlist:** a lead may only hire roles present in the catalog the team supplied, and an empty catalog (the default) grants no hiring tools at all. `src/pneuma/team.py:978-984`, `1426-1427`.
- **Inline agent synthesis is opt-in:** `dynamic_subagents` defaults to `False`, so `hire_dynamic` is absent from the wire entirely; when enabled, synthesized instructions are recorded verbatim in the roster log as the audit trail for a prompt nobody reviewed. `src/pneuma/team.py:832-851`, `552-558`.
- **Cross-member awareness is opt-in:** `worklog_enabled` defaults to `False`, and when on, delivery is step-boundary by construction — `notify` appends to a thread's log without starting a cycle, so a teammate is never interrupted mid-thought. `src/pneuma/team.py:853-868`, `1386-1400`.
- **Negotiation budget:** `negotiation_rounds` defaults to 0, which is the pre-negotiation skeleton exactly; a positive value bounds the phase and a round landing on the cap is marked `cap_reached` rather than implying consensus. `src/pneuma/team.py:814-829`, `1357-1359`.
- **Discovery vocabulary:** a member may only post one of four discovery kinds, and the poster's identity is bound by the tool rather than reported by the model, so attribution cannot be spoofed. `src/pneuma/team.py:629-637`, `725-733`, `755-757`.
- **Oracle-gated verdicts:** a `Team`'s lead answer must pass the subclass's oracle as a post-condition; `oracle` is required with no default precisely because a base that raised nothing would grade every verdict correct while reporting that it had been graded. `src/pneuma/team.py:962-976`.
