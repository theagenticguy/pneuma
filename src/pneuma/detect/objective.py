...
`probe` takes an objective, a declared feasible domain, and the shape of its answer space,
then sweeps, refines, and refuses. Five failure modes: `degenerate-optimum` and
`emptying-is-free` (a degenerate input is the optimum), `probe_feedback`'s mismatch (the
feedback states some quantity other than the score selection uses),
`raises-inside-the-domain`, `window-too-narrow` (the optimum sits on the swept window's own
edge), and an input escaping its declared range. Rationale and the worked measurements
behind every rule: `docs/design/objective.md`. The only pneuma import is `.discrimination`,
shared with `vacuity`; the rest is stdlib, so the arithmetic half runs offline. `search` is
the seam for `adversary.py`, never imported here. Four things break if edited carelessly.

**`Space` is required, never defaulted.** Metric axes vary freely, so any sane F-score is
maximised at the ideal corner — `coverage=1.0, edge_share=0.0` scores 1.0, the grid maximum,
on the face of the box — and a boundary-max check there fires on the sound objective and the
broken one alike. In decision space the same objective peaks interior at threshold 40 with
0.8613; swept over 1 to 24 it peaks at 24 with 0.8448, on the window's own edge.

**Out-of-domain inputs must not reward, and are neither clamped nor refused here.** The
escape this is built against put a pole at `edge_share == 1 + coverage`: unclamped,
`coverage=0.75, edge_share=2.0` scores 6.0 against an honest maximum of 1.0. Clamping
`edge_share=1.86` to 1.0 scores 0.0, identical to honest memorisation, so it hides that the
measurement was wrong rather than repairing it. `Domain.bounded_by` downgrades an escape to
a warning without skipping the check; `trust_declared_bounds=False` re-runs ignoring every
claim, since declaring it on every axis passes the unclamped objective with seven warnings.

**Degenerate inputs are computed from `Structure`, not declared.** A declared list alone
passed a genuinely degenerate objective with zero findings, on a transcript log where
coverage reads 0.0227 flat at every threshold from 1 to 44; on the permit log the smallest
surviving model scores 0.1496 against an optimum of 0.8606, so that gap was invisible there
by construction.

**The degenerate enumeration is decision-space only.** On metric axes 21 points tie for the
smallest non-zero `edge_share`, so the pick is `product`'s iteration order; tiebreaking on
score converges the emptiest point onto the ideal corner as the grid refines
(`coverage=1.0, edge_share=0.05` scores 0.9744 against a grid maximum of 1.0), eventually
refusing a sound objective for having a good optimum. In decision space 0.8184 at the argmax
falls to 0.7680 one step emptier (permit log), and every step to a single-edge model scores
0.0444 (transcript log).
"""
