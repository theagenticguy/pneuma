...
A drop-in alternative to `JSONMemoryBackend`. Three capabilities a learning loop needs
separately are one object here: targeted recall (entries addressable by id, so a gradient
lands on the few entries that bore on the decision rather than on one prose blob), one
artifact (parameters live in the same libSQL file as the event log, mined model, verdict,
and runs), and two learning channels (`GradFeedback.text` rewrites text entries,
`GradFeedback.score` moves numeric ones through `_numeric_update`, no separate host class).
Rationale, the retrieval measurement in full, and the gradient chain link by link:
`docs/design/turso_backend.md`. Four things break if edited carelessly.

**Never construct a bare cursor; read through `embedding.fetch_rows`.** `pyturso` 0.7.2
silently discards pending uncommitted writes when a `Cursor` holding an unfinalized SELECT
is garbage collected — `commit()` reports success and nothing raises. `Cursor.close()`
finalizes the active statement (lib.py:539); `__del__` does not. The symptom lands far from
the cause: a read-modify-write counter allocated the same id forever and surfaced as a
`UNIQUE constraint` failure on the entry table, three statements away. Full reproduction on
`embedding.fetch_rows`.

**Retrieval quality is measured, not assumed, and `distance_ceiling` must stay off by
default.** An embedding backend fails soft — a constant embedder still returns a full ranked
list, so every smoke test passes. `probe_retrieval` is a discrimination test: relevant
queries must land closer than deliberately unrelated ones, and overlapping distributions
refuse to certify rather than pass. Measured with Cohere Embed v4 over six agent-guidance
entries: 4/5 top-1, 5/5 at k=2 (which is why `learning.TOP_K > 1`), relevant mean 0.6303
against control mean 0.8683, separation +0.2381. `calibrate_ceiling` on those five probes
returns 0.7757 — a property of that corpus, not of the model, and no value anyone would
guess. A ceiling set by taste is a silent cap.

**Every retrieval query is an inner join, and nothing depends on FTS.**
`vector_distance_cos` raises `Conversion error: Invalid vector type` on a NULL blob rather
than returning NULL. Turso's FTS exists but `fts_score()` returns 0.0 for every matching
row, so it cannot rank; match syntax is `WHERE column MATCH ?`, not `fts_match(...)`.

**Entry ids come from a persisted monotonic counter and are never reused**, so an id
recorded in the forward pass still names the same logical entry at consolidation time
across saves, deletes, and reopens. A `ParameterView` is single-use — recall per call, not
once per batch — and must be passed as a handle, since a gradient target is discovered in
call arguments and `f"{view}"` drops the edge silently.
"""
