...
`objective.py` enumerates the degenerate inputs a declared `Structure` implies; an objective
can be gamed in a way nobody put in the declaration, and only a search finds those. This is
that search, as a `Search` callable `probe` accepts. Rationale and the full adjudication
argument: `docs/design/adversary.md`. Four things break if edited carelessly.

**This module stays out of `objective.py`.** That file imports nothing from pneuma and has no
network dependency, so a suite with no credentials runs every check it has. A Bedrock call
there would make the arithmetic half of the prober untestable offline. `probe(...,
search=None)` is the whole prober minus this, and that is the seam.

**The `Brief` must keep the tool that calls the objective, and `Brief.source` when the caller
supplies it.** Evaluating is what makes this a search rather than a guess: an adversary that
could only read a description reasons about what the arithmetic probably does. Source is how
a clamp hiding a mis-measurement, a guard tested with exact float equality, or a term that
cancels is found at all — sampling 21 points shows none of them.

**Adjudication is split, and the arithmetic half is not in this file.** A candidate becomes a
`degenerate-optimum` finding only if re-scoring it in `_check_degenerate` puts it at or above
the grid maximum, so `Verdict.upheld` is a comparison and a hallucinated triumphant input is
harmless. `judge` owns only the judgment call — whether an input is *worthless*, which its
score cannot decide, since a worthless input that scored badly would not be a defect.
`MIN_AGREEMENT` of `PANEL_SIZE` judges must say yes; a single judge agreeing with a single
proposer is two samples of the same prior. `Verdict` keeps every ballot so
`Panel.rejection_rate` reports how often judges said no — a panel that has never rejected
anything is a check that cannot fire.

**`ANGLES` is five prompts, not one prompt five times.** Identical adversaries explore the
neighbourhood their shared prior picks. The five — emptiness, escape, cancellation, clamp
exploitation, tie-seeking — are the five ways this project's own objective was observed to go
wrong, sourced from `docs/case-study.md` section 10, which is cited from code here and must
not be moved or renamed. No silent caps: the fan-out, `MAX_PER_ANGLE`, `PANEL_SIZE`, and
`MIN_AGREEMENT` are all arguments to `adversarial_search` and all land in the `Verdict`.

What this cannot do: the panel shares a great deal of prior with the proposers, and
`rejection_rate` measures that it *does* reject rather than proving it rejects the right
things. Nothing here would notice a panel and a proposer wrong in the same direction. The
arithmetic half has no such hole, which is why it decides whether a finding is recorded.
"""
