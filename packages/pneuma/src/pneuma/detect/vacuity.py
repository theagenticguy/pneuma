...
A model-checker answers whether any reachable state broke a rule, not whether the rule was
ever in a position to break, and both print green. The mechanism separating them is a
reachability sweep plus *relaxation*: re-sweep the same rules over a deliberately weakened
system, and if a rule still cannot break, nothing about the model's logic kept it safe.
Rationale: `docs/design/vacuity.md`. Four relaxations; the level at which a rule first
becomes breakable is its diagnosis.

    exact         as written, agrees with the checker. The rule can fire.
    free_initial  every variable starts at every value. A pinned initial value hid the
                  case  <-- the classic bug
    free_guards   transitions fire regardless of guards. A guard is load-bearing, pass is real
    free_both     both at once, the weakest system admitted. A pinned value still load-bearing
    no level      the rule cannot fire under any assignment: decoration

Three things break if edited carelessly.

**The relaxation order is load-bearing and `free_initial` must stay first.** Freeing an
initial value is a *sound* widening — what a TLA+ domain-membership clause in `Init` does,
with the model's own guards still enforcing themselves. Freeing guards is unsound, since it
walks edges the system forbids. So the sound one goes first, and a rule breakable under it is
diagnosed as hidden by a pinned variable even though ignoring guards would also have surfaced
it. That is what makes the gate correct: a checker pins initial values as the model does, so
a rule rescued only by `free_initial` was never checked and its pass must be withdrawn, while
one rescued by `free_guards` under the model's own initial values has a guard doing real work
and its pass stands. The widenings live in `adapter.py`'s walk; reordering here without
reordering the diagnosis there inverts the gate.

**`relaxation_truncated` is separate from `exact`'s `truncated`.** The levels bind at wildly
different sizes: with n free booleans `free_initial`'s start set is 2^n against `exact`'s one,
so a model can finish at `exact`, exhaust the budget at `free_initial`, and never sweep
`free_guards` — the level earning a guarded rule its pass. Without a separate flag that rule
reads as "0 violating, search finished, no witness", indistinguishable from decoration. The
flag makes the cause `unknown`, keeps `vacuous` False, and keeps the witness count 0 so the
pass is still withdrawn. `limit` likewise makes `live` and `vacuous` three-valued rather than
optimistic; a search that gave up is not evidence of safety.

**Nothing here imports its host project.** The consumer supplies a `System` and a list of
`Rule`s; `adapter.py` is the process-IR adapter and is the file you replace, not edit.
"""
